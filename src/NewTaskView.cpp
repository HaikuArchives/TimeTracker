...
NewTaskView::NewTaskView(BRect Frame)
	:
	BView(Frame, "NewTask", B_FOLLOW_ALL_SIDES, B_WILL_DRAW)
{
	...
	Temp.right -= 3;
	m_TextControl = new BTextControl(Temp, "TaskName", "", "", NULL,
		B_FOLLOW_LEFT_RIGHT);
	...
	m_Cancel = new BButton(Temp, "Cancel", "Cancel",
		new BMessage(BUTTON_NEW_TASK_CANCEL), B_FOLLOW_RIGHT | B_FOLLOW_TOP);
	AddChild(m_Cancel);
